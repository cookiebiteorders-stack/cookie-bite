-- =============================================================================
-- Cookie Bite — Migration 0118: Add granular error logging for order item insert
-- =============================================================================
-- This migration adds field-by-field error logging to the order item insert
-- to identify exactly which JSON column is causing the JSON syntax error
-- =============================================================================

-- Recreate the function with granular error logging
...
BEGIN
  ...
  FOR v_item_idx IN 0..jsonb_array_length(p_items)-1 LOOP
    ...
    -- Extract JSON fields with validation
    BEGIN
      v_product_snapshot := v_item->'product_snapshot';
    EXCEPTION
      WHEN OTHERS THEN
        RETURN QUERY SELECT NULL::uuid, NULL::text, NULL::text, false, ('product_snapshot_extract_failed:' || v_slug)::text;
        RETURN;
    END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RETURN QUERY SELECT NULL::uuid, NULL::text, NULL::text, false, ('variant_snapshot_extract_failed:' || v_slug)::text;
        RETURN;
    END;
    ...
    EXCEPTION
      WHEN OTHERS THEN
        RETURN QUERY SELECT NULL::uuid, NULL::text, NULL::text, false, ('selected_addons_extract_failed:' || v_slug)::text;
        RETURN;
    END;
