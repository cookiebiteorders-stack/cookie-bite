-- =============================================================================
-- Cookie Bite — Migration 0122: Add detailed JSON debug logging
-- =============================================================================
-- This migration adds detailed RAISE NOTICE logging to identify exactly
-- which JSON field is causing the "invalid input syntax for type json" error
-- =============================================================================

-- Recreate the function with detailed JSON debug logging
...
BEGIN
  ...
  FOR v_item_idx IN 0..jsonb_array_length(p_items)-1 LOOP
    ...
    -- Extract JSON fields directly using -> operator with debug logging
    BEGIN
      v_product_snapshot := v_item->'product_snapshot';
      RAISE NOTICE '[JSON-DEBUG] %: product_snapshot type=% value=%', v_slug, jsonb_typeof(v_product_snapshot), v_product_snapshot;
    EXCEPTION
      WHEN OTHERS THEN
      v_product_snapshot := NULL;
      RAISE NOTICE '[JSON-DEBUG] %: product_snapshot extraction failed, using NULL', v_slug;
    END;

    BEGIN
      v_variant_snapshot := v_item->'variant_snapshot';
      RAISE NOTICE '[JSON-DEBUG] %: variant_snapshot type=% value=%', v_slug, jsonb_typeof(v_variant_snapshot), v_variant_snapshot;
    EXCEPTION
      WHEN OTHERS THEN
      v_variant_snapshot := NULL;
      RAISE NOTICE '[JSON-DEBUG] %: variant_snapshot extraction failed, using NULL', v_slug;
    END;

    BEGIN
      v_selected_addons := v_item->'selected_addons';
      RAISE NOTICE '[JSON-DEBUG] %: selected_addons type=% value=%', v_slug, jsonb_typeof(v_selected_addons), v_selected_addons;
    EXCEPTION
      WHEN OTHERS THEN
      v_selected_addons := NULL;
      RAISE NOTICE '[JSON-DEBUG] %: selected_addons extraction failed, using NULL', v_slug;
    END;

    -- Insert order item with direct JSON extraction (-> operator)
    BEGIN
      ...
      VALUES (
        ...
        v_product_snapshot,          -- Direct extraction, can be null
        CASE WHEN (v_item->>'variant_id') IS NOT NULL AND (v_item->>'variant_id') != '' THEN (v_item->>'variant_id')::uuid ELSE NULL END,
        v_variant_snapshot,          -- Direct extraction, can be null
        v_selected_addons,           -- Direct extraction, can be null
        now()
      );
      RAISE NOTICE '[JSON-DEBUG] %: order_item inserted successfully', v_slug;
    EXCEPTION
      WHEN OTHERS THEN
      v_error_detail := SQLERRM;
      RAISE NOTICE '[JSON-DEBUG] %: order_item insert failed: %', v_slug, v_error_detail;
