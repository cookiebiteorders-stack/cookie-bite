-- =============================================================================
-- Cookie Bite — Migration 0119: Fix JSON operator usage in order item insert
-- =============================================================================
-- This migration fixes the JSON operator usage in the order item insert.
-- The issue was using ->> (text extraction) instead of -> (JSON extraction)
-- for JSONB columns like product_snapshot, variant_snapshot, selected_addons
-- =============================================================================

-- Recreate the function with correct JSON operators
...
BEGIN
  ...
  FOR v_item_idx IN 0..jsonb_array_length(p_items)-1 LOOP
    ...
    -- Insert order item with correct JSON operators (-> not ->>)
    BEGIN
      ...
      VALUES (
        ...
        v_item->'product_snapshot',           -- Use -> for JSONB column
        CASE WHEN (v_item->>'variant_id') IS NOT NULL AND (v_item->>'variant_id') != '' THEN (v_item->>'variant_id')::uuid ELSE NULL END,
        v_item->'variant_snapshot',           -- Use -> for JSONB column
        v_item->'selected_addons',            -- Use -> for JSONB column
        now()
      );
    EXCEPTION
      WHEN OTHERS THEN
