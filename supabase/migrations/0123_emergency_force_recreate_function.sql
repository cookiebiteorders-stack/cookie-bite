-- =============================================================================
-- Cookie Bite — Emergency Fix: Force recreate checkout function
-- =============================================================================
-- This script directly executes the SQL to recreate the function with proper JSON handling
-- =============================================================================

-- First, check if the function exists and drop it
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'create_checkout_order_transactional') THEN
    DROP FUNCTION public.create_checkout_order_transactional CASCADE;
    RAISE NOTICE 'Function dropped successfully';
  END IF;
END $$;

-- Recreate the function with simplified JSON handling
...
BEGIN
  ...
  -- Insert order items with direct JSON extraction
  FOR v_item_idx IN 0..jsonb_array_length(p_items)-1 LOOP
    v_item := p_items->v_item_idx;
    v_slug := v_item->>'slug';
    v_quantity := (v_item->>'quantity')::integer;
    ...
    -- Insert order item - use direct JSON extraction in VALUES clause
    BEGIN
      ...
      VALUES (
        ...
        v_item->'product_snapshot',           -- Direct JSON extraction
        CASE WHEN (v_item->>'variant_id') IS NOT NULL AND (v_item->>'variant_id') != '' THEN (v_item->>'variant_id')::uuid ELSE NULL END,
        v_item->'variant_snapshot',           -- Direct JSON extraction
        v_item->'selected_addons',            -- Direct JSON extraction
        now()
      );
    EXCEPTION
      WHEN OTHERS THEN
