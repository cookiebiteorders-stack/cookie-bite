...
async function sendLastOrderEmail() {
  console.log('=== إرسال إيميل آخر طلب للمالكين فعلياً ===\n');
  ...
  try {
    ...
    const fromEmail = process.env.RESEND_FROM_EMAIL || 'cookie-bite@cookie-bite.com';
    const subject = `طلب جديد #${lastOrder.order_number} - ${lastOrder.status.toUpperCase()}`;
    ...
    for (const ownerEmail of ownerEmails) {
      try {
        ...
        console.log(`✅ Email sent to ${ownerEmail}:`, result.id);
        successCount++;
      } catch (error) {
