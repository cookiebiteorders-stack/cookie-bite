...
import { Command, RefreshCw, Search, Sparkles, Upload } from "lucide-react";
...
export function ProductsCommandPalette({
  open,
  onClose,
  onAddProduct,
  onFocusSearch,
  ...
  const items = useMemo<CommandItem[]>(
    () => [
      { id: "add", label: "إضافة منتج", hint: "N", icon: Sparkles, action: onAddProduct },
  ...
  const filtered = useMemo(() => {
    ...
    if (!s) return items;
    return items.filter((i) => i.label.toLowerCase().includes(s) || i.id.includes(s));
  }, [items, q]);
