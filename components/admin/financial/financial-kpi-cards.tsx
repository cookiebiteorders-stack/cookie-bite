        ...
        {cards.map((c, i) => (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <p className="text-[11px] font-bold uppercase tracking-wide text-cb-text-muted">{c.label}</p>
              <c.icon className="h-4 w-4 shrink-0 text-cb-text-muted" />
            </div>
            <p className="mt-2 font-serif text-xl font-bold tabular-nums text-cb-text-strong sm:text-2xl">
              {c.value}
            </p>
            <p className="text-xs text-cb-text-muted">{c.sub}</p>
